...
class TTS:
    def __init__(
        ...
        models_dir: str = Path.home() / ".cache" / "gsv",
        device: str = None,
        is_half: bool = None,
        ...
        self.cnhubert_path = os.path.join(self.models_dir,"chinese-hubert-base")
        self.cnroberta_path = os.path.join(self.models_dir,"chinese-roberta-wwm-ext-large")
        self.sv_path = os.path.join(self.models_dir,"sv\pretrained_eres2netv2w24s4ep4.ckpt")
        self.default_gpt_path = os.path.join(self.models_dir,"s1v3.ckpt")
        self.default_sovits_path = os.path.join(self.models_dir,"v2Pro\s2Gv2ProPlus.pth")
        ...
        if use_bert and not os.path.exists(self.cnroberta_path):
            download_model(
                ...
                zip_filename=os.path.join(self.models_dir,"chinese-roberta-wwm-ext-large.zip")
            )
    ...
    def load_gpt_model(self, *model_paths: str):
        ...
        """
        if not model_paths:
            model_paths = (os.path.join(self.models_dir, "s1v3.ckpt"),)
    ...
    def load_sovits_model(self, *model_paths: str):
        ...
        """
        if not model_paths:
            model_paths = (os.path.join(self.models_dir, "v2Pro\s2Gv2ProPlus.pth"),)
        for model_path in model_paths:
