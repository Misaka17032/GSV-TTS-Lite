...
class T2SBlock(nn.Module):
    ...
    def process_prompt(
        ...
        k = k.view(batch_size, k.shape[1], self.num_heads, -1)
        v = v.view(batch_size, v.shape[1], self.num_heads, -1)

        k_cache[:, :kv_cache_len] = k
        v_cache[:, :kv_cache_len] = v

        attn = F.scaled_dot_product_attention(q.transpose(1, 2), k.transpose(1, 2), v.transpose(1, 2), attn_mask)
    ...
    def decode_next_token(
        ...
        k_cache.index_copy_(1, kv_cache_len, k)
        v_cache.index_copy_(1, kv_cache_len, v)

        # kv_cache shape [batch_size, kv_len, num_heads, head_dim/num_heads]

        attn = flash_attn_with_kvcache(q, k_cache, v_cache, cache_seqlens=kv_cache_len.to(torch.int32), causal=True)

        attn = attn.reshape(batch_size, q_len, -1)
        attn = F.linear(attn, self.out_w, self.out_b)
    ...
    def decode_next_token(
        ...
        for i in range(self.num_blocks):
            x = self.blocks[i].decode_next_token(
                x, k_cache[i], v_cache[i], kv_cache_len
            )
        kv_cache_len += 1
        return x
...
class Text2SemanticDecoder(nn.Module):
    ...
    def warmup(self, dtype, device, max_kv_caches):
        ...
        for max_kv_cache in max_kv_caches:
            ...
            self.t2s_transformer.decode_next_token(bucket.graph_xy_pos, bucket.k_cache, bucket.v_cache, bucket.kv_cache_len)
            
            bucket.cuda_graph = torch.cuda.CUDAGraph()
            with torch.cuda.graph(bucket.cuda_graph):
                bucket.graph_xy_dec = self.t2s_transformer.decode_next_token(bucket.graph_xy_pos, bucket.k_cache, bucket.v_cache, bucket.kv_cache_len)
